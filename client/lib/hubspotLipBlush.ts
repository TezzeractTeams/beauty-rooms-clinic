 ...
 * Set in .env: VITE_HUBSPOT_PORTAL_ID, VITE_HUBSPOT_LIP_BLUSH_FORM_GUID
 */
const portalId = import.meta.env.VITE_HUBSPOT_PORTAL_ID as string | undefined;
const formGuid = import.meta.env.VITE_HUBSPOT_LIP_BLUSH_FORM_GUID as string | undefined;

export function isHubSpotLipBlushConfigured(): boolean {
  return Boolean(portalId?.trim() && formGuid?.trim());
}

export type LipBlushLeadPayload = {
  firstName: string;
  lastName: string;
  email: string;
  phone: string;
...
export async function submitLipBlushLead(
  payload: LipBlushLeadPayload,
): Promise<{ ok: true } | { ok: false; message: string }> {
  if (!portalId?.trim() || !formGuid?.trim()) {
    return {
      ok: false,
      message:
        "This form is not connected yet. Add VITE_HUBSPOT_PORTAL_ID and VITE_HUBSPOT_LIP_BLUSH_FORM_GUID to your environment.",
    };
  }
  ...
  const body = {
    ...
    context: {
      ...
      pageName: "Effortless Lip Blush & Tint Launch — Beauty Rooms Clinic",
    },
  };
