...
const HUBSPOT_EMBED_SCRIPT_SRC = "https://js-na2.hsforms.net/forms/embed/245575840.js";
const HUBSPOT_EMBED_SCRIPT_ID = "hs-forms-embed-work-with-us";

export function ProfessionalIntakeFormSection() {
  useEffect(() => {
  ...
  return (
    <section
      ...
      className="w-full scroll-mt-24 px-6 py-20 md:px-10 md:py-28 lg:py-32"
    ...
    >
      <div className="mx-auto max-w-7xl">
        ...
        <div className="border border-[rgba(103,92,83,0.12)] p-8 shadow-[0_1px_0_rgba(255,255,255,0.5)_inset] md:p-10 [color-scheme:light]">
          <div
            ...
            data-region="na2"
            data-form-id="135159f8-7476-4e11-9668-5a493d2ffc17"
      ...
      {/*
      Previous custom intake form (replaced by HubSpot embed above).
      ...
      >
        <div>
          ...
          <input id="intake-name" name="name" type="text" autoComplete="name" placeholder="Full name" className={inputClass} />
        </div>
        <div className="grid gap-8 md:grid-cols-2 md:gap-6">
          <div>
            <label htmlFor="intake-email" className={labelClass}>Email</label>
            <input id="intake-email" name="email" type="email" autoComplete="email" placeholder="you@example.com" className={inputClass} />
          </div>
          <div>
            <label htmlFor="intake-phone" className={labelClass}>Telephone number</label>
            <input id="intake-phone" name="phone" type="tel" autoComplete="tel" placeholder="(941) 555-0100" className={inputClass} />
          </div>
        </div>
        <div>
          <label htmlFor="intake-social" className={labelClass}>Social media</label>
          <input id="intake-social" name="socialMedia" type="text" placeholder="Instagram, LinkedIn, or profile URL" className={inputClass} />
        </div>
        <div>
          <label htmlFor="intake-profession" className={labelClass}>Profession</label>
          <input id="intake-profession" name="profession" type="text" list="intake-profession-suggestions" placeholder="Your primary role or specialty" className={inputClass} />
          <datalist id="intake-profession-suggestions">{professionSuggestions.map((s) => (<option key={s} value={s} />))}</datalist>
        </div>
        <div>
          <label htmlFor="intake-license" className={labelClass}>License held</label>
          <textarea id="intake-license" name="licenseHeld" rows={3} placeholder="License type, number, state, and expiry if applicable" className={`${inputClass} min-h-[6.5rem] resize-y py-3`} />
        </div>
        <div>
          <label htmlFor="intake-resume" className={labelClass}>Resume</label>
          ...
          <p className="mt-2 font-barlow text-xs font-normal text-charcoal/45">PDF or Word, typically under 5 MB when submission is enabled.</p>
        </div>
        <div className="pt-2">
          <button
            type="button"
